...
> 本文是 **1. LLM Fundamentals（LLM 基础）** 的第四课。前三课我们有了数学、Python 和神经网络，这一课把它们全部用到**人类语言**上：让机器完成分类、理解、翻译乃至生成文本。
>
...
| --- | --- |
| 文本 → 向量 | 第一课线性代数；本课词嵌入 |
| 分词 → 词典 → 词频 | 第二课 Pandas / sklearn |
| 文本分类 | 第三课神经网络 / 第二课逻辑回归 |
| 预测下一个词 | 第一课概率（MLE）+ 交叉熵（信息论） |
| 注意力打分 | 第一课点积 + softmax |
...
> 🎯 到这里，Transformer 的全部核心部件已经齐了：**嵌入 → 多头自注意力 → 前馈 MLP → 位置编码 → 掩码**。GPT 就是把这些层堆很多很多遍。第四课的信息量到这里已经非常接近"能读懂 GPT 论文"了。
...
| 任务 | 常用指标 | 直觉 |
| --- | --- | --- |
| 文本分类 | 准确率、精确率、召回率、F1 | 第二课学过：垃圾过滤看重精确率 |
| 语言模型 | **困惑度（Perplexity）** | 模型平均在几个词之间犹豫（第一课信息论） |
...
> 🎯 有没有发现？这张图的每一块，都是我们第四课（以及前三课）**亲手实现过或用数学推导过的东西**。LLM 不神秘——它就是"把本章所有积木，堆到巨大的规模"。
...
**到这里，1. LLM Fundamentals 的四课全部完成**：数学 🧮 → Python 🐍 → 神经网络 🧠 → NLP 📖。下一站是《4. 大模型基础》与 **2. The LLM Scientist**——从理论走向"亲手构建 LLM"。我们继续。😀
