...
| [`04-训练体系`](04-训练体系) | 数据 → 预训练 → 对齐 → 微调 → 评测（已开篇 00 全链路总览 + 01 预训练数据工程 + 02 合成数据与数据飞轮） | §4 |
