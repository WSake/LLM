...
| [`08-RAG体系`](08-RAG体系) ⭐ | 检索 → 重排 → GraphRAG → Agentic RAG → 评测（已开篇 02 核心流水线·总览 + 01 检索基础 + 03 混合重排 + 04 分块元数据 + 05 查询侧 + 06 上下文压缩 + 07 RAGAS 评测 + 08 向量数据库选型·先定场景再选库 + 09 Advanced 模式 + 10 Agentic RAG 决策层 + 11 GraphRAG 实体图谱） | §8 |
