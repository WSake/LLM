...
- **后启**：`04-低代码`（✅ 已交付 v0.30：类 5 低代码平台——数据分析/无码不再是管线，是积木化+全兜底，账 C 四痛点专表铨实）；`05-Haystack`（账 A 双栖点别二：生产级 RAG 管线的 pipeline 思维，与本文对照读）；`09-DSPy`（Prompt 编程）；`12-继承关系与选型决策`（收束章）。
