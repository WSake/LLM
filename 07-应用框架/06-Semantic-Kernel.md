...
| `07-AutoGen·AG2` | §7.8 背书：微软 2025 推出 Microsoft Agent Framework 承接 AutoGen、与 SK 统一 | ⬜ |
...
- **后启**：`07-AutoGen·AG2`（§7.8 官方背书：SK 与 AutoGen 的统一）；`11-MCP协议`（本篇账 D=协议层的 SK 出口，milestone `013` 仍在 11-章）；`10-OpenAI-Agents-SDK`（回归轻量对照）；`12-继承关系与选型决策`（收束章：完整 DAG + "该不该引入"量化）。
