...
- **后启**：`00-框架分类学`（先把 8 类框架的"该在哪用"钉死，本篇只是图引擎一格的展开）；`01-LangChain与历史包袱`（同为 LangChain 系，链 vs 图的分界）；`12-框架继承关系与选型决策`（收束章"该不该引入框架"——本篇 C/D 账给出两条判据：要不要可恢复（checkpoint）、要不要可循环（自环/上限））；`07-AutoGen-AG2-Microsoft-Agent-Framework`·`08-CrewAI`·`10-OpenAI-Agents-SDK与轻量运行时`（Agent 系：同是这张图，换 LLM 节点、加多体编排）。
