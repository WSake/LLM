# 🐍 Python 机器学习：从数据处理到模型实战

> 本文是 **1. LLM Fundamentals（LLM 基础）** 的第二课。上一课我们用数学搭好了骨架（线性代数、微积分、概率统计、信息论），这一课把骨架填成肉：**用 Python 亲手完成"拿到数据 → 清洗 → 可视化 → 建模型 → 评估 → 用起来"的完整流程**。
>
...
3. **进阶**：学完本课后进入第 3 课《神经网络》，把 `sklearn.fit` 换成 `PyTorch` 的自定义训练循环。
...
**本课与第一课的关系**：第一课给你"为什么"（数学原理），本课给你"怎么做"（Python 生态）。下一课《神经网络》将把 torch 引入，从零搭出真正的深度学习模型。准备好，我们继续。😀
